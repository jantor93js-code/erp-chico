...
function loadDatabaseUrlFromEnv(envPath: string): string {
...
function postImport(payload: unknown): Promise<{ status: number; body: any }> {
...
async function getPrismaDocument(codigo: string) {
  return prisma.document.findUnique({
    where: { codigo },
  });
}

async function deletePrismaDocumentIfExists(codigo: string): Promise<Document | null> {
...
async function restorePrismaDocument(original: Document | null) {
...
function printFailure(step: string, expected: unknown, actual: unknown, stack: string) {
  console.log(`FAILED`);
