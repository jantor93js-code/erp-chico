...
async getGraficas() {
  ...
  const gastos =
    await this.prisma.gastoOperativo.findMany();
...
const facturadoMes =
  ...
  const pedidosMes =
  await this.prisma.pedido.count({
    where: {
      createdAt: {
        gte: inicioMes,
        lte: finMes,
  ...
  const serviciosMes =
  await this.prisma.servicio.count({
    where: {
      createdAt: {
        gte: inicioMes,
        lte: finMes,
  ...
  const conductores =
    await this.prisma.conductor.count();
...
const utilidadOperativa =
  (totalFacturado._sum.valor || 0) -
  (gastosOperativos._sum.valor || 0);

const serviciosRentables =
...
if (serviciosRentables.length > 0) {
  const suma =
    serviciosRentables.reduce(
      (total, servicio) => {

        const gastos =
          servicio.gastosOperativos.reduce(
            (s, gasto) => s + gasto.valor,
            0,
          );
  ...
  rentabilidadPromedio =
    Math.round(
      suma / serviciosRentables.length,
    );
}
