...
export default function AreasManagement() {
  ...
  const loadAreas = useCallback(async () => {
    setLoading(true);
    try {
      const result = await areasService.getAll(pagination.skip, pagination.take);
  ...
  const handleSearch = async (query: string) => {
    ...
    setLoading(true);
    try {
      const results = await areasService.search(query);
      setAreas(results);
    } catch (error) {
      toast.error('Error al buscar áreas');
    } finally {
      setLoading(false);
    }
  };
      ...
      {areas.length > 0 && (
        <div className="flex justify-between items-center pt-4">
          <p className="text-sm text-gray-600">
            Mostrando {areas.length} de {pagination.total} áreas
          </p>
